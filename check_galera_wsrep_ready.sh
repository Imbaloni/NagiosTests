...
if [[ "$1" = "--help" ]] || [[ "$1" = "-h" ]]; then
    echo "This script checks wsrep_ready with a critical state"
    echo "How to use: "
    echo "./check_galera_wsrep_ready.sh -u USER -p PASS "
    echo "Whereas there is a critcal if it is off"
fi
while (( "$#" )); do
   case $1 in
        -u|--user)
...
query=$(mysql --user $USER --password\=$PASS -s -N -e "show status like 'wsrep_ready';")
connectedVar=$(echo "${query:11}" | tr '[:upper:]' '[:lower:]')
echo $connectedVar
