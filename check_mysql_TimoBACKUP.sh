...
#first timer stops here, if the result didn't fetch anything in time or is empty a critical message will be fired
STOP1=$(date +0%s.%3N);
...
if (( $(echo "$DURATION > $CRITCAL" |bc -l 2>/dev/null) )); then
    ...
    if  ! mysql -D $DATABASE --user $USER --password\=$PASS -se "select * from "$TABLENAME"; " > /dev/null 2>&1 ; then
        ...
        #time will be stopped a second time, if the table is slow or doesn't respond there will be a warning
        STOP2=$(date +%s.%3N);
        DURATION=$(echo "((($STOP2-1) - $START))" | bc)
        echo $(echo "$DURATION < $CRITCAL" |bc -l) 
